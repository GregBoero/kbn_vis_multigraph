...
module.controller('kbnVisMultipleGraphController', ($scope, $element, $rootScope) => {
    ...
    let gen_chart_opt = (label, type, color) => {
        return {label: label, type: type, color: color};
    };
    ...
    $scope.chartGen = () => {
        ...
        _.each(theLabels, (chart, index) => {
            switch (index) {
                case 0:
                    dataColors[chart] = $scope.vis.params.chart_color;
                    dataTypes[chart] = $scope.vis.params.chart_type;
                    break;
                case 1:
                    dataColors[chart] = $scope.vis.params.chart_color;
                    dataTypes[chart] = $scope.vis.params.chart_type;
        ...
        if (tableGroups && tableGroups.tables) {
            _.each(tableGroups.tables, (table) => {
                _.each(table.columns, (column, index) => {
                    ...
                    if (index > 0) {
                        $scope.vis.params.charts_option.push(gen_chart_opt(column.title, $scope.vis.params.chart_type, $scope.vis.params.chart_color));
